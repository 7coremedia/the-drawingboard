...
interface BehanceStyleEditorProps {
  mode: "create" | "edit";
...
export default function BehanceStyleEditor({ mode, initialMeta, onSubmit, isLoading }: BehanceStyleEditorProps) {
  ...
  const form = useForm<MetaFormData>({
    ...
    defaultValues: {
      ...
      client: initialMeta?.client ?? "",
      category: initialMeta?.category ?? "Branding",
      tagline: initialMeta?.tagline ?? "",
      year: initialMeta?.year ?? "",
      industry: initialMeta?.industry ?? "",
      location: initialMeta?.location ?? "",
      our_role: initialMeta?.our_role ?? "",
      the_challenge: initialMeta?.the_challenge ?? "",
      the_solution: initialMeta?.the_solution ?? "",
      description: initialMeta?.description ?? "",
      notes: initialMeta?.notes ?? "",
      is_notes_downloadable: initialMeta?.is_notes_downloadable ?? true,
  ...
  const watchPublished = form.watch("is_published");
  const watchMultiPartner = form.watch("is_multiple_partners");
  ...
  const handleSubmit = form.handleSubmit(async (data) => {
    ...
    try {
      ...
      const isCaseStudy = blocks.length === 1 && pdfBlock;

      await onSubmit({
        ...data,
        media_url: coverImage,
        full_image_url: coverImage,
        content_blocks: blocks,
        portfolio_type: isCaseStudy ? "case_study" : "gallery",
    ...
    <div className="flex flex-col h-[calc(100vh-64px)] bg-[#F5F0E8] overflow-hidden">
      ...
      <header className="flex items-center h-14 px-6 bg-white/90 backdrop-blur-md border-b border-black/5 shrink-0 z-50 gap-4">
        ...
        <div className="flex items-center gap-3 ml-auto shrink-0">
          ...
          <button
            type="button"
            onClick={() => form.setValue("is_published", !watchPublished)}
            className={cn(
        ...
        >
          <div className="flex-1 overflow-y-auto p-6 space-y-5 min-w-[288px]">
            ...
            <div>
              ...
              >
                ...
                <SelectContent className="bg-white border-black/5 rounded-2xl">
                  {["Branding", "Logo", "Poster", "Other"].map((c) => (
            ...
            <div>
              ...
              <Textarea {...form.register("tagline")} placeholder="Core project statement..." className={fieldTextarea} />
            </div>
            ...
            <div>
              ...
              <Input {...form.register("industry")} placeholder="e.g. Fintech, Fashion..." className={fieldInput} />
            </div>
            <div>
              <label className={fieldLabel}>Geographic Scope</label>
              <Input {...form.register("location")} placeholder="Lagos, Global..." className={fieldInput} />
            </div>
            <div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between py-2">
                <div>
                  <div className="text-[9px] font-black uppercase tracking-[0.3em] text-black/70">Public Export</div>
                  <div className="text-[8px] font-bold text-black/30">Allow doc downloads</div>
                </div>
                <Switch
                  checked={form.watch("is_notes_downloadable")}
                  onCheckedChange={(v) => form.setValue("is_notes_downloadable", v)}
                  className="data-[state=checked]:bg-[#C94A2C]"
                />
              </div>
              <div className="flex items-center justify-between py-2">
                <div>
                  <div className="text-[9px] font-black uppercase tracking-[0.3em] text-black/70">Collaborative</div>
                  <div className="text-[8px] font-bold text-black/30">Multiple partners</div>
                </div>
                <Switch
                  checked={watchMultiPartner}
                  onCheckedChange={(v) => form.setValue("is_multiple_partners", v)}
                  className="data-[state=checked]:bg-[#C94A2C]"
                />
              </div>
            </div>
            ...
            {watchMultiPartner && (
              <div>
                <label className={fieldLabel}>Collaborative Project Name</label>
                <Input {...form.register("brand_name")} placeholder="Project identity label" className={fieldInput} />
              </div>
            )}
