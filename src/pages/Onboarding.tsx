...
const VisualSelectorCard = ({
  ...
  return (
    <div
      className={`relative cursor-pointer rounded-xl border-2 p-4 transition-all hover:border-primary ${selected ? "border-primary bg-primary/5" : "border-border"
        }`}
      onClick={onSelect}
    >
            ...
            <FormItem>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {colorPalettes.map((palette) => (
                  <div
                    key={palette.id}
                    className={`relative cursor-pointer rounded-xl border-2 p-4 transition-all hover:border-primary ${field.value === palette.id ? "border-primary bg-primary/5" : "border-border"
                      }`}
            ...
            <FormItem>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {typographyPairings.map((pairing) => (
                  <div
                    key={pairing.id}
                    className={`relative cursor-pointer rounded-xl border-2 p-4 transition-all hover:border-primary ${field.value === pairing.id ? "border-primary bg-primary/5" : "border-border"
                      }`}
...
const FinalStep = () => (
  <motion.div className="text-center" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}>
    <CircleCheckBig className="mx-auto h-16 w-16 text-[#0b00ff]" />
